...
static void UpdateNote(Mod_t* Mod_p, uint8_t* Data_p);
...
static void SetSample(uint16_t Channel, uint8_t Sample, uint32_t SampleOffset);
...
uint8_t InitMOD(CDFileHandle_t *File_p, Mod_t* Mod_p)
{
  ...
  Mod_p->PatternsBuff_p = (uint8_t*)malloc(Mod_p->NumberOfPatterns * MOD_ROWS_PER_CHANNEL * Mod_p->NumberOfChannels * 6);
  if (!Mod_p->PatternsBuff_p)
    return 4; /* couldn't allocate pattern buffer */

  /* Pattern data starts at offset 1084 - read and decode into pattern buffer */
  if ( File_p->Read(File_p, Data_p, Mod_p->NumberOfPatterns * MOD_ROWS_PER_CHANNEL * Mod_p->NumberOfChannels * 4) !=
                                   (Mod_p->NumberOfPatterns * MOD_ROWS_PER_CHANNEL * Mod_p->NumberOfChannels * 4) )
  {
  ...
  Iz = 0;
  for ( Ix = 0; Ix < Mod_p->NumberOfPatterns; Ix++ )
  {
    for ( Iy = 0; Iy < MOD_ROWS_PER_CHANNEL; Iy++ )
    {
        uint32_t *Div_p = (uint32_t *)&Data_p[(Ix * MOD_ROWS_PER_CHANNEL * Mod_p->NumberOfChannels * 4) + (Iy * Mod_p->NumberOfChannels * 4)];
        uint8_t Channel;
        for ( Channel = 0; Channel < Mod_p->NumberOfChannels; Channel++ )
        {
          ...
          */
          SampleNumber = ((division >> 24) & 0x00F0) | ((division >> 12) & 0x000F);
          PeriodFrequency = ((division >> 16) & 0x0FFF);
          EffectNumber = (division >> 8) & 0x000F;
          EffectParameter = division & 0x00FF;
          NoteColumn = 0xFF; // note doesn't exist
          /* look up table here */
          for ( Cnt = 0; Cnt < MOD_NUMBER_OF_NOTES; Cnt++ )
          {
            if ( AmigaPeriodsTable[ 0 ][ Cnt ] == Mod_p->Notes[ Channel ].PeriodFrequency )
            {
              NoteColumn = Cnt; // note exists
              break;
            }
          } /* for Cnt */
          Mod_p->PatternsBuff_p[Iz + 0] = SampleNumber;
          Mod_p->PatternsBuff_p[Iz + 1] = EffectNumber;
          Mod_p->PatternsBuff_p[Iz + 2] = EffectParameter;
          Mod_p->PatternsBuff_p[Iz + 3] = NoteColumn;
          Mod_p->PatternsBuff_p[Iz + 4] = PeriodFrequency >> 8;
          Mod_p->PatternsBuff_p[Iz + 5] = PeriodFrequency & 0x00FF;
          Iz += 6;
        }
      }
  }
  ...
  for ( Sample = 0, Iw = 0; Sample < MOD_NUMBER_OF_SAMPLES; Sample++ )
  {
    if ( Mod_p->Inst[ Sample ].SampleLength > 0 )
    {
      ...
      if (Mod_p->Inst[ Sample ].Factor)
      {
        ...
        while (Soffset > 0)
        {
          for (Ix = 0, Iz = 0; (Ix < (1 << Mod_p->Inst[ Sample ].Factor)) && Soffset; Ix++, Soffset--)
            Iz += File_p->Get(File_p);
          Data_p[Iy++] = Iz >> Mod_p->Inst[ Sample ].Factor;
        }
      }
      else
...
uint16_t PlayMOD(Mod_t* Mod_p)
{
  ...
  if (Mod_p->Tick >= Mod_p->Speed)
  {
    ...
    {
      UpdateNote( Mod_p, &Mod_p->PatternsBuff_p[ Mod_p->Pattern * MOD_ROWS_PER_CHANNEL * Mod_p->NumberOfChannels + Mod_p->Row * Mod_p->NumberOfChannels * 6 ] ); /* process 1 note for every channel */
...
static void UpdateNote(Mod_t* Mod_p, uint8_t* Data_p)
{
  ...
  for ( Channel = 0; Channel < Mod_p->NumberOfChannels; Channel++ )
  {
    Mod_p->Notes[ Channel ].SampleNumber = Data_p[0];
    Mod_p->Notes[ Channel ].EffectNumber = Data_p[1];
    Mod_p->Notes[ Channel ].EffectParameter = Data_p[2];
    Mod_p->Notes[ Channel ].NoteExists = (Data_p[3] == 0xFF) ? FALSE : TRUE;
    Mod_p->Notes[ Channel ].PeriodFrequencyCol = (Data_p[3] == 0xFF) ? 0 : Data_p[3];
    Mod_p->Notes[ Channel ].PeriodFrequency = (Data_p[4] << 8) + Data_p[5];
...
//        }
    ...
    if ( Mod_p->Notes[ Channel ].NoteExists == TRUE )
    {
      ...
      {
        /* set sample */
        SetSample( Channel, Mod_p->Channels[ Channel ].LastInstrument, Mod_p->Channels[ Channel ].SampleOffset );
      }
    } /* if */
  } /* for Channel */
...
static void SetSample(uint16_t Channel, uint8_t Sample, uint32_t SampleOffset)
{
  uint16_t loop = gMod->Inst[ Sample ].LoopStart >> gMod->Inst[ Sample ].Factor;
  voices[Channel].start = gMod->Inst[ Sample ].SampleHandle;
  voices[Channel].offset = SampleOffset >> gMod->Inst[ Sample ].Factor;
  voices[Channel].loop = loop ? loop : (gMod->Inst[ Sample ].SampleLength >> gMod->Inst[ Sample ].Factor) - 1;
...
static void SetVolume(uint16_t Channel, uint32_t Volume, int32_t Pan)
{
  ...
  pan = pcm_lcf(Pan); // Linear Cross Fade pan value
#else
  if ( (chan == 0) || (chan == 3) )
  {
#ifdef CROSS_TALK
...
static void callback(void)
{
  ...
  {
    pcm_set_ctrl(0x40 + i); // turn off pcm chip briefly and select channel
  ...
  pcm_set_ctrl(0xC0); // restart pcm chip
}
